...
<div class="purdue-gradient relative min-h-screen overflow-hidden">
  ...
  <div class="relative z-10 flex min-h-screen items-center justify-center p-4">
    <div class="glass-effect w-full max-w-xl rounded-3xl p-8">
      ...
      <!-- Welcome Message -->
      <div class="mb-8">
        <h2 class="mb-3 text-2xl font-semibold text-white">Welcome back, dormmate!</h2>
        <p class="text-sm leading-relaxed text-gray-300">Your perfect match awaits in this legendary adventure!</p>
      </div>

      <form class="flex-1 space-y-6" {...login}>
        {#if errors.general}
        ...
        <div class="flex w-full justify-between">
          <Checkbox label="Remember me" id="remember-me-checkbox" name="rememberMe" />
          <a href={resolve("/auth/register")} class="text-accent-light transition-colors duration-300 hover:text-accent-light/80">
            First time here?
          </a>
        </div>
        ...
        {#if !login.result?.ok && !!login.result?.message}
          <p class="mt-1 text-sm text-red-400">{login.result.message}</p>
        {/if}
      </form>
