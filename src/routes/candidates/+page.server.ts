import { getSwipes } from "$lib/server/functions";
import { error } from "@sveltejs/kit";

export const load = async () => {
  const [left, right] = await Promise.all([getSwipes("left"), getSwipes("right")]);
  if (!left.ok) {
  ...
  return { candidates: left.data, rejects: right.data };
  // return {
  //     candidates: [
