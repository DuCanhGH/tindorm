<script lang="ts">
  ...
  function onScrollHeader() {
    ...
    if (y > lastY && y > 160)
      hidden = true; // scrolling down past threshold → hide
...
<div class="min-h-screen bg-gradient-to-b from-black/100 to-yellow-500">
  ...
  <FrontBalls />
  <header
    class={`fixed top-0 right-0 left-0 z-50 transition-transform duration-300 ease-out ${hidden ? "-translate-y-full" : ""}`}
    style="will-change: transform;"
  >
    <div class="px-4 py-3">
      <div
        class={`mx-auto max-w-5xl rounded-2xl border border-white/15 shadow-lg backdrop-blur transition-all duration-300 ease-out ${compact ? "bg-white/5" : "bg-white/10"}`}
          ...
          <div class="flex w-full items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <button
                ...
                class="rounded-full bg-white px-4 py-2 text-sm font-medium text-black shadow transition hover:bg-white/90"
              >
                Donate
              </button>
              <button
            ...
            <a href="/auth/login" class="rounded-full border border-white/30 px-4 py-2 text-sm text-white/90 transition hover:bg-white/10">
              Log in
            </a>
          </div>
        </nav>
  ...
  <main class="mx-auto flex w-full max-w-5xl flex-1 flex-col items-center gap-6 px-4 pb-16">
    <div class="relative mt-4 mt-20 h-[70vh] w-full max-w-xl select-none">
      {#if false}
        <!-- lock cover -->
